...
const ViewLeave = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        const response = await axios.get(`http://localhost:5000/leave/getLeaveRequestsByUserId/${userId}`, {
  ...
  const handleEdit = (id) => {
    navigate('/employee/editLeave', { state: { requestId: id } });
  };
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`http://localhost:5000/leave/deleteLeaveRequest/${deleteId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      setRequests(requests.filter(request => request._id !== deleteId)); // Ensure correct ID is used
      setShowModal(false);
    } catch (error) {
  ...
  return (
    <div className="container mt-5">
      ...
      <h2 className="text-center text-primary">Leave Requests</h2>
